...
- **`status=trash`** on `/posts` and `/pages`. `any` keeps its historical meaning (everything except the trash), so a consumer that wants trashed content must ask for it explicitly. `any` expands to every registered non-internal status instead of a fixed list, so posts sitting in a status registered by another plugin still reach a full export — including the reserved workflow statuses that editorial plugins declare with `exclude_from_search`, which `WP_Query`'s own `'any'` silently drops.
